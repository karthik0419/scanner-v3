...
def test_telegram_bots():
    ...
    if os.path.exists(env_path):
        with open(env_path) as f:
            env_content = f.read()
        if "TELEGRAM_BOT_ID_REDACTED" in env_content and "TELEGRAM_CHAT_ID_REDACTED" in env_content:
            test_pass(".env has SwingU bot (token TELEGRAM_BOT_ID_REDACTED, chat TELEGRAM_CHAT_ID_REDACTED)")
        else:
