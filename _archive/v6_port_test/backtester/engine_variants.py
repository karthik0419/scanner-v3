"""
Backtest engine with entry filter variant support.
Same as engine_with_filter.py but uses entry_filter_variants.py.
"""
import os, sys
import pandas as pd
import numpy as np
...
from entry_filter_variants import score_entry
...
def backtest_symbol(symbol, years=2, min_score=50, scan_every=5,
                    atr_stop=True, entry_filter_threshold=0, filter_variant='v1_original'):
            ...
            if days_since_sl <= RE_ENTRY_MAX_DAYS and close >= last_breakout:
                if i + 1 < len(df):
                    ...
                    if re_entry_stop < re_entry_price:
                        open_trade = {
                            "symbol": symbol, "pattern": "Re-entry",
                            "signal_date": current_date, "entry_date": df.index[i + 1],
                            "entry_price": re_entry_price, "stop_loss": re_entry_stop,
                            ...
                            "breakout_level": last_breakout, "entry_conf_score": -1,
                        }
                        last_breakout = 0
                        last_sl_date = None
                        last_scan_idx = i
            ...
            if result:
                ...
                if score >= min_score and rr > 0:
                    ...
                    if entry_filter_threshold > 0:
                        conf_score, _ = score_entry(df_slice, bo_val, variant=filter_variant)
